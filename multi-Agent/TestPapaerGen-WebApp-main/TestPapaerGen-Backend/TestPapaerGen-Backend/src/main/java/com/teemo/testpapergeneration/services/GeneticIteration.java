...
//    }
    ...
    // 构造函数
    public GeneticIteration(Integer iterationsNum, List<QuestionBank> questionList, double targetDifficult, Integer TKTCount, Integer XZTCount, Integer PDTCount) {
...
//        int n = TKTCurrent.size() + XZTCurrent.size() + PDTCurrent.size() + JDTCurrent.size();
        int n = TKTCurrent.size() + XZTCurrent.size() + PDTCurrent.size();
