...
#描述
根据{summary_content}的书本知识点内容，生成选择题考试题目，主章节为{label_1_content}和次章节为{label_2_content}，题目数量为{num_content}，题目难度为{difficulty_content}。
...
题目：进程的同步与互斥是由于程序的（）引起的。A顺序执行；B长短不同；C信号量；D并发执行
答案：D.并发执行
